...
import UmdWarningModal from "../components/modal/umd-warning-modal";
...
function initializeUmdWarningBuilder(api) {
  ...
  api.modifyClass("controller:composer", {
    ...
    async save(...args) {
      ...
      if (model.umd_checkUnformattedMarkdownDetected() &&
        !model.umd_checkShouldIgnoreWarning()) 
      {
        //const result = await this.modal.show(MyModal, { model });
        const result = await getOwner(this)
                          .lookup("service:modal")
                          .show(UmdWarningModal, { model });
        
        _this.umd_closeModal.call(_this);

        if (result) { // ignoreAndProceed 
...
export default {
  ...
  initialize() {
    withPluginApi("0.8.8", initializeUmdWarningBuilder);
  },
};